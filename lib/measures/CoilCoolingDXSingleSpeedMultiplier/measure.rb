...
class CoilCoolingDXSingleSpeedMultiplier < OpenStudio::Measure::ModelMeasure
  ...
  def change_name(object, rated_cop_multiplier, rated_cooling_capacity_multiplier)
    ...
    if rated_cop_multiplier != 1.0
      nameString += " #{rated_cop_multiplier.round(2)}x coilEff"
    end
    if rated_cooling_capacity_multiplier != 1.0
      nameString += " #{rated_cooling_capacity_multiplier.round(2)}x coilCap"
    end
...
CoilCoolingDXSingleSpeedMultiplier.new.registerWithApplication
