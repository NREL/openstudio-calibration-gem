...
class CoilHeatingGasPercentChange < OpenStudio::Ruleset::ModelUserScript
  ...
  # human readable name
  def name
    return "Heating Coils Gas Percent Change"
  end

  # human readable description
  def description
    return "This is a general purpose measure to calibrate Gas Heating Coils with a Percent Change."
  end
  ...
  def change_name(object,coil_parasitic_gas_perc_change,coil_efficiency_perc_change,coil_parasitic_electric_perc_change,coil_capacity_perc_change)
    ...
    if coil_parasitic_gas_perc_change != 1.0
      nameString = nameString + " #{coil_parasitic_gas_perc_change.round(2)}percng gasPara"
    end
    if coil_parasitic_electric_perc_change != 1.0
      nameString = nameString + " #{coil_parasitic_electric_perc_change.round(2)}percng elecPara"
    end
