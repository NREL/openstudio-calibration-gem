...
class FansPercentChange < OpenStudio::Ruleset::ModelUserScript
  ...
  # human readable name
  def name
    return "Fans Percent Change"
  end

  # human readable description
  def description
    return "This is a general purpose measure to calibrate Fans with a Percent Change."
  end
  ...
  def change_name(object,max_flowrate_perc_change,fan_efficiency_perc_change,pressure_rise_perc_change,motor_efficiency_perc_change)
    ...
    if max_flowrate_perc_change != 0.0
      nameString = nameString + " #{max_flowrate_perc_change.round(2)}percng flow"
    end
    if pressure_rise_perc_change != 0.0
      nameString = nameString + " #{pressure_rise_perc_change.round(2)}percng press"
    end
    if fan_efficiency_perc_change != 0.0
      nameString = nameString + " #{fan_efficiency_perc_change.round(2)}percng fanEff"
    end
    if motor_efficiency_perc_change != 0.0
      nameString = nameString + " #{motor_efficiency_perc_change.round(2)}percng motorEff"
    end
