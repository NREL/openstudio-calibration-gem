...
class CoilCoolingWaterPercentChange < OpenStudio::Measure::ModelMeasure
  ...
  def change_name(object, design_water_flow_rate, design_air_flow_rate, design_inlet_water_temperature, design_inlet_air_temperature, design_outlet_air_temperature, design_inlet_air_humidity_ratio)
    ...
    if design_water_flow_rate != 1.0
      nameString += " #{design_water_flow_rate.round(2)} percng waterDFR"
    end
    if design_air_flow_rate != 1.0
      nameString += " #{design_air_flow_rate.round(2)}percng airDFR"
    end
    if design_inlet_water_temperature != 1.0
      nameString += " #{design_inlet_water_temperature.round(2)}percng DIWT"
    end
    if design_inlet_air_temperature != 1.0
      nameString += " #{design_inlet_air_temperature.round(2)}percng DIAT"
    end
    if design_outlet_air_temperature != 1.0
      nameString += " #{design_outlet_air_temperature.round(2)}percng DOAT"
    end
    if design_inlet_air_humidity_ratio != 1.0
      nameString += " #{design_inlet_air_humidity_ratio.round(2)}percng DIAHR"
    end
...
CoilCoolingWaterPercentChange.new.registerWithApplication
